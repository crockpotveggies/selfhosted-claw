...
**Codex OAuth token** (`CLAUDE_CODE_OAUTH_TOKEN=...` or `ANTHROPIC_AUTH_TOKEN=...`):
```bash
...
1. **Codex subscription (Pro/Max)** — description: "Uses your existing Codex Pro or Max subscription. You'll run `Codex setup-token` in another terminal to get your token."
2. **Anthropic API key** — description: "Pay-per-use API key from console.anthropic.com."

#### Subscription path

Tell the user to run `Codex setup-token` in another terminal and copy the token it outputs. Do NOT collect the token in chat.
