...
- **Identity & Memory** â€” paths of files created, which AGENTS.md was edited
...
- **Simpler config:** NanoClaw has no config file ? behavior is in the code and `AGENTS.md` files. Credentials live in `.env` or integration-specific host settings.
...
1. **Shared personality (recommended if your groups had the same bot)** â€” "I'll put the shared personality, identity, and user context in `groups/global/AGENTS.md`. Every group sees it. Each group can add its own customizations on top."
...
Before registering groups, confirm the assistant name â€” it's used for trigger patterns and AGENTS.md templates.
...
The register step's `--assistant-name` flag writes `ASSISTANT_NAME` to `.env` and updates AGENTS.md templates automatically â€” no manual `.env` write needed.
...
The assistant name was already confirmed and written to `.env` in Phase 1. Here, focus on the rest of the identity â€” create an `identity.md` file with the full identity details (emoji, creature, vibe, personality traits, etc.). If shared personality was chosen in Phase 1, put it alongside `groups/global/AGENTS.md`. Otherwise, put it in `groups/main/`.

### SOUL.md

Read `<STATE_DIR>/workspace/SOUL.md` if it exists. Then read `groups/main/AGENTS.md`.

AGENTS.md is always loaded into the agent's context â€” it's the agent's continuous instructions. Not everything from SOUL.md needs to be there. Discuss with the user what belongs where:

- **In AGENTS.md (always loaded):** Core personality traits, communication style, key behavioral rules. Weave these into the existing AGENTS.md structure â€” adjust the opening description under the `# <Name>` heading, modify the tone in the Communication section.
...
- Shared personality â†’ edit `groups/global/AGENTS.md` for the core traits, create `groups/global/soul.md` for the extended content. All groups will see both.
- Separate / main only â†’ edit `groups/main/AGENTS.md`, create `groups/main/soul.md`.
...
3. **If the OpenClaw plugin was a CLI tool** â€” read the config to identify the exact tool. If it's an npm package, add it to the container's Dockerfile. Add a note to the group's AGENTS.md that the tool is available and how to invoke it.
...
If MCP_SERVERS was non-empty in discovery, these can be ported. Codex supports MCP servers natively. Read the OpenClaw config's `mcp.servers` section to get each server's details (`command`, `args`, `env`, `url`).
...
- **Compaction:** Handled by Codex SDK automatically
...
**Migrated:**
- Assistant name â†’ `.env` ASSISTANT_NAME + AGENTS.md templates updated
- Groups â†’ registered in database, folders created with AGENTS.md templates
- Timezone â†’ `.env` TZ
- Anthropic credential â†’ `.env` (for setup to pick up)
- Sender allowlists â†’ `~/.config/nanoclaw/sender-allowlist.json`
- Personality â†’ AGENTS.md (core) + `soul.md` (extended), placed per Phase 1 decision (global or per-group)
