...
1. **Check for existing work.** Search open PRs and issues before starting:
   ```bash
   gh pr list --repo crockpotveggies/selfhosted-claw --search "<your feature>"
   gh issue list --repo crockpotveggies/selfhosted-claw --search "<your feature>"
   ```
...
Self-Hosted Claw uses [Claude Code skills](https://code.claude.com/docs/en/skills) — markdown files with optional supporting files that teach Claude how to do something. There are four types of skills in Self-Hosted Claw, each serving a different purpose.
...
Add capabilities to Self-Hosted Claw by merging a git branch. The SKILL.md contains setup instructions; the actual code lives on a `skill/*` branch.
...
1. Fork `crockpotveggies/selfhosted-claw` and branch from `main`
